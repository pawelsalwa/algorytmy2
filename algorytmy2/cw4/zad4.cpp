...
//        	}
	...
	bool MS[iloscWezlow+1][iloscWezlow+1]; //maciez sasiedztwa, wypelniam zerami.

	for(int i =1; i<= iloscWezlow; i++){
		for(int j =1; j<= iloscWezlow; j++){
			MS[i][j] = 0;
		}
	}
	
	for(int i =1; i<= iloscPolaczen; i++){	//wypelniam maciez sasiedztwa jedynkami
